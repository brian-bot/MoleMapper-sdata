...
#####
moleMapperRegisterIDU <- function(userName, pathToIDU){
  ...
  addThis <- data.frame(userName = userName,
                        ...
                        iduRegisteredBy = synGetUserProfile()@userName,
...
#####
moleMapperAcceptIDU <- function(principalId, iduId){
  
  ## GET THE REGISTRATION TABLE
  td <- synTableQuery(paste0("SELECT * FROM syn7210333 WHERE principalId='", principalId, "' AND iduId='", iduId, "'"))
  res <- td@values
  ...
  accessRestrictionLevel3 <- "7209087"
  ...
  res$iduAccepted <- TRUE
  res$iduAcceptedBy <- synGetUserProfile()@userName
  res$iduAcceptedDate = as.character(Sys.Date())
  td@values <- res
